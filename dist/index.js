...
var run = () => __async(exports, null, function* () {
  ...
  const githubApiKey = core.getInput("GITHUB_TOKEN") || core.getInput("github_token") || process.env.GITHUB_TOKEN;
  const octokit = (0, import_github.getOctokit)(githubApiKey);
  const latestVersion = yield getLatestReleaseTag(octokit);
  console.log(`Latest release version ${latestVersion}`);
  if (isReRelease(version, latestVersion)) {
    handleRerelease();
  }
  if (isRollback(version, latestVersion)) {
    yield handleRollback(octokit, version);
  }
  if (isRelease(version, latestVersion)) {
    yield handleRelease(version, octokit, latestVersion);
  }
});
function handleRelease(version, octokit, latestVersion) {
  return __async(this, null, function* () {
    ...
    const { name, body } = yield getNotes(octokit, version, latestVersion);
    const changelogText = transformReleaseNotes(body);
    yield createRelease(octokit, changelogText, name, version);
  });
}
function handleRollback(octokit, version) {
  return __async(this, null, function* () {
    console.log("Release type: rollback");
    core.setOutput("release_type", "rollback");
    const latestRelease = yield deleteReleases(octokit, version);
    if (import_semver.default.lt(latestRelease.tag_name, version)) {
      console.log(`Creating rollback release for version ${version}`);
      const { name, body } = yield getNotes(octokit, version, latestRelease.tag_name);
      const changelogText = transformReleaseNotes(body);
      yield createRelease(octokit, changelogText, name, version);
    }
  });
...
function isRelease(version, latestVersion) {
  return import_semver.default.gt(version, latestVersion);
}
function isReRelease(version, latestVersion) {
  return import_semver.default.eq(version, latestVersion);
}
function isRollback(version, latestVersion) {
  return import_semver.default.lt(version, latestVersion);
}
function deleteReleases(octokit, version) {
  return __async(this, null, function* () {
    ...
    for (const release of releases) {
      if (import_semver.default.gt(release.tag_name, version)) {
        yield deleteRelease(octokit, release.id);
        console.log(`Removed release ${release.tag_name}`);
        count++;
      }
      if (import_semver.default.lte(release.tag_name, version)) {
        return release;
      }
    }
...
function deleteRelease(octokit, id) {
  return __async(this, null, function* () {
    yield octokit.request("DELETE /repos/{owner}/{repo}/releases/{release_id}", {
      owner,
      repo,
      release_id: id
    });
...
function getLatestReleaseTag(octokit) {
  return __async(this, null, function* () {
    try {
      const { data } = yield octokit.request("GET /repos/{owner}/{repo}/releases/latest", {
        owner,
        repo
      });
...
function transformReleaseNotes(body) {
  const jiraIssueRegex = /([a-z]{3,10}-[0-9]+)/gi;
  let notes = body.split("\n").map((line) => {
  ...
  const releaseNotes = notes.replace(jiraIssueRegex, (match) => `[${match}](https://24so.atlassian.net/browse/${match})`);
  console.log("Final release notes:");
  console.log(releaseNotes);
  return releaseNotes;
}
function createRelease(octokit, changelogText, name, version) {
  return __async(this, null, function* () {
...
function getNotes(octokit, version, previousVersion) {
  return __async(this, null, function* () {
    const {
      data: { name, body }
    } = yield octokit.request("POST /repos/{owner}/{repo}/releases/generate-notes", {
      owner,
      repo,
      tag_name: version,
      previous_tag_name: previousVersion === "v0.0.0" ? void 0 : previousVersion
    });
    return { name, body };
  });
}
